#!/usr/bin/env python3
"""
Final bulk translation script that properly saves to Supabase
"""
...
def get_total_food_count() -> int:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(data) == 1000:
                return 1000  # We'll handle pagination in the main loop
            else:
                return len(data)
        return 0
    except Exception as e:
...
def main():
    ...
    print(f"📊 Estimated foods: {total_foods:,}")
    ...
    while offset < total_foods:
        ...
        if offset >= 100:  # Stop after 100 foods for testing
